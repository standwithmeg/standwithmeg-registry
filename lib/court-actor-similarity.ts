...
function normalizeRole(value: string | null | undefined): string | null {
  if (!value) return null;
  const role = normalizeCourtActorRoleLabel(value).toLowerCase().replace(/\s+/g, " ").trim();
  return role || null;
}
...
export function buildSuggestedClusters(
  ...
  for (const row of rows) {
    ...
    const counted = row.review_decision !== "duplicate"
      && row.review_decision !== "submitter_hidden";
    if (counted) {
    ...
    variant.samples.push({
      ...
      // merge_comments rows still count; only their original note is hidden.
