...
function collapseRepeatedLetters(value: string): string {
  // Only collapse 3+ identical repeats (e.g. "pattt" -> "pat"). Legitimate doubles
  // in real names (Patterson, Griffith, Curley, etc.) must be preserved in keys.
  return value
    .split(" ")
    .map(token => token.length >= 5 ? token.replace(/([a-z])\1{2,}/g, "$1") : token)
    .join(" ");
}
...
export function publicActorRoleLabel(role: string | null | undefined): string {
  const clean = normalizeCourtActorRoleLabel(role);
  if (!clean) return "Court Actor";
  return clean;
}

export function publicActorRoleEntries(roles: Map<string, number>): Array<[string, number]> {
  const publicRoles = new Map<string, number>();
  for (const [role, count] of roles.entries()) {
    const label = publicActorRoleLabel(role);
    publicRoles.set(label, (publicRoles.get(label) ?? 0) + count);
  }
  return Array.from(publicRoles.entries()).sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]));
}
 ...
 *   merge_comments   — keep the row countable, but hide its original public
 *                      note because its testimony has been folded into a
 *                      primary row's court_actor_comment_merges.merged_comment.
 *   submitter_hidden — original row preserved, but the submitter removed it
 *                      during a later survey update.
 *   null/undefined   — normal: row counts using email|location dedupe.
 */
export type CourtActorRowReviewDecision = "duplicate" | "count_separately" | "merge_comments" | "submitter_hidden" | null | undefined;

/**
 * Compute the family-dedupe key for a court_actors row.
 *   - returns null when the row is marked duplicate or submitter_hidden
...
export function resolveFamilyKey(args: {
  ...
  if (args.review_decision === "submitter_hidden") return null;
...
import { normalizeCourtActorRoleLabel } from "./court-actor-roles";
