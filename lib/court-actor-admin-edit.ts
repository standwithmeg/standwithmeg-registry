import { isCourtActorRoleOption, normalizeCourtActorRoleLabel } from "./court-actor-roles";
...
export function normalizeCourtActorEditFields(fields: unknown): { update: CourtActorEditUpdate } | { error: string } {
  ...
  if (Object.prototype.hasOwnProperty.call(raw, "role")) {
    ...
    const role = normalizeCourtActorRoleLabel(raw.role);
    if (!role) return { error: "role cannot be empty." };
    if (!isCourtActorRoleOption(role)) return { error: "role must be selected from the court actor role list." };
    update.role = role;
  }
