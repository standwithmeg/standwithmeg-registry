...
export async function getDocumentDraftingContext(documentType, state, caseType, userFacts = {}) {
  ...
  // Load state-specific statutes from the state_data.json database
...
async function loadStateStatutes(state, _caseType) {
  try {
    // Load the state database we built earlier
    const stateData = await import("../state_data.json", { assert: { type: "json" } });
    const stateInfo = stateData.states.find(s => s.abbr === state);
    if (!stateInfo) return [];
    return stateInfo.records_access_statutes || [];
  } catch {
    return [];
  }
}
...
export async function adaptTemplateForState(templateType, state) {
  ...
  let stateData;
  try {
    const data = await import("../state_data.json", { assert: { type: "json" } });
    stateData = data.states.find(s => s.abbr === state);
  } catch {
    stateData = null;
  }
