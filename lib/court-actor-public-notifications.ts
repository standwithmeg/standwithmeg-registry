...
function createNotificationSupabaseClient() {
  if (typeof window !== "undefined") {
    throw new Error("Notification Supabase client must only be used on the server.");
  }
...
type AdminClient = ReturnType<typeof createNotificationSupabaseClient>;
...
export async function getPublicActorsWithReporters(): Promise<PublicActorBucket[]> {
  ...
  while (true) {
    const { data, error } = await sb
      ...
      // Stable unique sort key — range pagination drops/duplicates rows near
      // page boundaries without it, undercounting families past 1000 rows.
...
/**
 * Build the locked photo-request email body. Simply asks the reporter
 * to look for and reply with a public photo of the named court actor.
 * Never reveals other reporters or how many families submitted.
 */
export function buildPhotoRequestEmail(args: {
  ...
  const stateLabel = args.locationKey.trim().toUpperCase() || locationDisplay;
  const subject = `Photo needed: ${args.canonicalName} (${stateLabel})`;
  const body = [
    `Hi ${greetingName},`,
    "",
    `Please look for a public photo of ${args.canonicalName} (${locationDisplay}) and reply to this email with the photo or a link to one.`,
    "",
    "Reply in this thread so I can match it to the right court actor.",
    "",
    "Thank you,",
    "Meg",
    "Stand With Meg",
...
export async function dispatchPendingPhotoRequests(
  ...
  const { transporter } = await createSmtpTransport("Court actor photo request", {
  ...
  for (const p of plan) {
    ...
    try {
      ...
      let { error: insertErr } = await sb.from("court_actor_public_notifications").insert(notificationRow);
      if (insertErr) {
        ({ error: insertErr } = await sb.from("court_actor_public_notifications").insert(notificationRow));
      }
      if (insertErr) {
        logger.error(
          `[fail] sent email but could not record notification for ${p.reporter.reporter_email}: ${insertErr.message}`,
        );
        summary.failed += 1;
        summary.details.push({
          kind: "failed",
          ...
          canonical_name: p.bucket.canonical_name,
          location_key: p.bucket.location_key,
          error: `SMTP ok but DB record failed: ${insertErr.message}`,
        });
        continue;
      }
      summary.sent += 1;
