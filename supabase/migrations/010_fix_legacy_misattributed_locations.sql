...
--
-- This migration fixed them surgically by email + first_name (emails redacted
-- after the repo went public; the migration already ran in production) so we do
...
-- 2. Legacy row A — Edmonton, Alberta (this is Canada, not Alabama)
update legacy_submissions
set state_of_occurrence = null,
    outside_us_country  = 'Canada'
where lower(email) = 'redacted-legacy-a@removed.invalid'
  and outside_us_country in ('CA', 'ca');

-- 3. Legacy row B — county literally says "Canada"
update legacy_submissions
set state_of_occurrence = null,
    outside_us_country  = 'Canada'
where lower(email) = 'redacted-legacy-b@removed.invalid'
  and case_county = 'Canada';

-- 4. Legacy row C — Ohio (Guernsey county). outside="GG" is garbage.
update legacy_submissions
set outside_us_country = null
where lower(email) = 'redacted-legacy-c@removed.invalid'
  and state_of_occurrence = 'OH'
  and outside_us_country in ('GG', 'gg');

-- 5. Legacy row D — Maryland. outside="CA" is garbage.
update legacy_submissions
set outside_us_country = null
where lower(email) = 'redacted-legacy-d@removed.invalid'
  and state_of_occurrence = 'MD'
  and outside_us_country in ('CA', 'ca');

-- 6. Legacy row E — Oregon, Benton county. outside="AS" is garbage.
update legacy_submissions
set outside_us_country = null
where lower(email) = 'redacted-legacy-e@removed.invalid'
