...
import { COURT_ACTOR_ROLE_OPTIONS } from "../lib/court-actor-roles";
...
const ROLE_PATTERNS: Array<{ regex: RegExp; role: string; requireTwoWord?: boolean }> = [
  ...
  { regex: new RegExp(`\\b(?:Attorney|attorney|ATTORNEY|Lawyer|lawyer|LAWYER|Counsel|counsel)\\s+${NAME_STRICT}`, "g"), role: "Attorney" },
...
async function callAIForExtraction(text: string, subId: string): Promise<AIActor[]> {
  ...
  const systemPrompt = `You are an assistant extracting named court actors from family-court survey responses. Extract proper names of real people or specifically named institutions/agencies (judges, attorneys, GALs, evaluators, CPS workers, therapists, doctors, hospitals, agencies). Ignore first names alone unless clearly tied to a role. Return JSON. Do not include speculative names.

Allowed roles (choose best match):
${COURT_ACTOR_ROLE_OPTIONS.map(role => `- ${role}`).join("\n")}`;
