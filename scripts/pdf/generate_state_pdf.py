...
def state_input_hash(state_abbr: str, rows: list, court_actors: list, version_hash: str, source: str, sponsor: dict | None = None) -> str:
    payload = {
        ...
        "source": source,
        # Regenerate when the cover sponsor changes (added/removed/edited).
...
_SYSTEM_NORM = {
    ...
    'family court': 'Family Court Only',
    'cps': 'CPS Only',
    'cps (child protective services) only': 'CPS Only',
...
_CASE_STATUS_NORM = {
    ...
    'i was a child of family court or cps': 'Experienced as a child',
    ...
    'i am not currently in court but still affected': 'Not in court but still affected',
}
...
def normalize_duration(v):
    ...
    if not s:
        return ''
    return _DURATION_NORM.get(s.lower(), s)
...
_INVALID_LOCATION_VALUES = {'None', 'nan', '', 'State of Occurrence', 'Unsure'}


def _title_case_location(raw: str) -> str:
    tokens = raw.lower().split()
    small = {"of", "the", "and", "in", "for"}
    return " ".join(
        token if idx > 0 and token in small else token[:1].upper() + token[1:]
        for idx, token in enumerate(tokens)
    )


def normalize_report_location(raw: str) -> str:
    location = " ".join(safe_str(raw).split())
    if location in _INVALID_LOCATION_VALUES:
        return ""
    upper = location.upper()
    if upper in STATE_NAMES:
        return upper
    return _title_case_location(location)


def is_us_state_location(location: str) -> bool:
    return location.upper() in STATE_NAMES


def report_location_name(location: str) -> str:
    upper = location.upper()
    if upper in STATE_NAMES:
...
# Cover banner image per location. US states and any location not listed
# below fall back to the default US-flag banner. Add a country here (keyed
# by its uppercased name) to give it its own flag-themed cover image, and
...
def cover_image_for(location: str) -> str:
    """Return the cover banner asset filename for a location."""
    return COVER_IMAGE_BY_LOCATION.get(location.upper(), DEFAULT_COVER_IMAGE)


def build_states(rows):
    by_state = defaultdict(list)
    for row in rows:
        s = normalize_report_location(row[1])
        if s:
            by_state[s].append(row)
    return by_state
...
def state_stats(rows):
    ...
    def topic_values(k):
        if k == 'custody':
            norm = normalize_custody
        elif k == 'county':
            norm = normalize_county
        elif k == 'system':
            norm = normalize_system
        elif k == 'case_status':
            norm = normalize_case_status
        elif k == 'duration':
            norm = normalize_duration
        else:
            norm = safe_str
        values = []
        for r in rows:
    ...
    def top(k, t=8):
        c = Counter(topic_values(k))
        return c.most_common(t)

    def answered_count(k):
        return len(topic_values(k))

    pro_se_answers = [
        safe_str(r[COLS['pro_se']])
        for r in rows
        if COLS['pro_se'] < len(r) and safe_str(r[COLS['pro_se']])
    ]
    legal_rep_answers = [
        safe_str(r[COLS['legal_rep']])
        for r in rows
        if COLS['legal_rep'] < len(r) and safe_str(r[COLS['legal_rep']])
    ]
    prose = sum(1 for answer in pro_se_answers if 'yes' in answer.lower() or 'pro se' in answer.lower())
    ran = sum(1 for answer in legal_rep_answers if 'ran out' in answer.lower())
    ...
    return {
        'total': n,
        'pro_se_count': prose,
        'pro_se_n': len(pro_se_answers),
        'pro_se_pct': round(prose / len(pro_se_answers) * 100, 1) if pro_se_answers else 0,
        'ran_out_count': ran,
        'legal_rep_n': len(legal_rep_answers),
        'ran_out_pct': round(ran / len(legal_rep_answers) * 100, 1) if legal_rep_answers else 0,
        'financial': financial,
        ...
        'case_status_n': answered_count('case_status'),
        'system_n': answered_count('system'),
        'duration_n': answered_count('duration'),
        'custody_n': answered_count('custody'),
        'allegations_n': answered_count('allegation'),
        'counties_n': answered_count('county'),
...
def _sponsor_context(sponsor):
    """Shape a raw sponsors-table row into the cover-band template vars.

    Resolves the logo to an absolute filesystem path (under public/) so the
    Playwright/WeasyPrint renderer can load it via file://. Returns None when
    there is no sponsor, so the template simply omits the band.
    """
    if not sponsor:
        return None
    logo_rel = (sponsor.get("logo_url") or "").lstrip("/")
    logo_file = WEBSITE_ROOT / "public" / logo_rel if logo_rel else None
    logo_path = str(logo_file) if logo_file and logo_file.exists() else None

    website_url = sponsor.get("website_url") or ""
    website_label = (
        website_url.replace("https://", "").replace("http://", "").rstrip("/") or None
    )
    return {
        "business_name": sponsor.get("business_name"),
        "tagline": sponsor.get("tagline"),
        "services": sponsor.get("services"),
        "location_label": sponsor.get("location_label"),
        "phone": sponsor.get("phone"),
        "website_url": website_url or None,
        "website_label": website_label,
        "brand_color": sponsor.get("brand_color") or "#C9A227",
        "logo_path": logo_path,
    }
...
def build_template_context(state_abbr, rows, court_actors=None, sponsor=None):
    ...
    f = d['financial']
    ml = d['months_lost']
    is_us_state = is_us_state_location(state_abbr)
    state_name = report_location_name(state_abbr)
    ...
    state_number = sorted_states.index(state_abbr.upper()) + 1 if is_us_state else None

    return {
        'state_name': state_name,
        'state_abbr': state_abbr,
        'is_us_state': is_us_state,
        'cover_image': cover_image_for(state_abbr),
        ...
        # Bar chart sections
        'case_status': _bar_items(d['case_status'][:5], d['case_status_n']),
        'system': _bar_items(d['system'][:5], d['system_n']),
        'duration': _bar_items(d['duration'][:5], d['duration_n']),
        'custody': _bar_items(d['custody'][:5], d['custody_n']),
        'counties': _bar_items(d['counties'][:6], d['counties_n']),
        'allegations': _bar_items(d['allegations'][:5], d['allegations_n']),
        'case_status_n': d['case_status_n'],
        'system_n': d['system_n'],
        'duration_n': d['duration_n'],
        'custody_n': d['custody_n'],
        'counties_n': d['counties_n'],
        'allegations_n': d['allegations_n'],
        ...
        # State Exclusive sponsor lockup on the cover (None hides the band)
        'sponsor': _sponsor_context(sponsor),
    }


def generate_pdf(state_abbr, rows, output_path=None, browser=None, court_actors=None, sponsor=None):
    """Render the Jinja2 template and convert to PDF via Playwright."""
    ctx = build_template_context(state_abbr, rows, court_actors=court_actors, sponsor=sponsor)
...
def main():
    ...
    # Bootstrap Supabase env from .env.local for local runs (CI passes real
    # env vars, and setdefault never overrides those).
    ...
    load_state_exclusive_sponsor = None
    if use_supabase:
        from lib_supabase_rows import (
        ...
        public_actors_by_state = {
            normalize_report_location(k): v
            for k, v in load_public_court_actors_from_supabase().items()
        }
    else:
    ...
    if only_30plus:
        ...
        print(f"  --all-30plus: {len(states)} locations qualify")
    else:
        states = [normalize_report_location(s) for s in args]
    ...
    for state in states:
        ...
        sponsor = load_state_exclusive_sponsor(state) if load_state_exclusive_sponsor else None
        input_hash = state_input_hash(state, sr, court_actors, version_hash, source_name, sponsor)
        ...
        if not force and cached and cached.get("input_hash") == input_hash and (PUBLIC_STATE_REPORTS_DIR / f"{state}.pdf").exists():
            ...
            continue
        if sponsor:
            print(f"    Sponsor: {sponsor.get('business_name')} (State Exclusive)")
        print(f"\n  Generating {report_location_name(state)} ({state}) — {len(sr)} submissions")
        generate_pdf(state, sr, court_actors=court_actors, sponsor=sponsor)
        cache[state] = {
