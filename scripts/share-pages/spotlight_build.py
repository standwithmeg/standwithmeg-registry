...
def select(table: str, columns: list[str], filters: list[str] | None = None, limit: int | None = None, order: str | None = None, offset: int | None = None) -> list[dict]:
...
def select_all(table: str, columns: list[str], filters: list[str] | None = None, order: str = "id.asc") -> list[dict]:
    """Page through every matching row. PostgREST caps un-paged selects at
    1000 rows and truncates silently past the cap, which would quietly drop
    admin review decisions or actor rows. `order` must include a unique
    column (default: primary key) so pages never skip or duplicate rows.
    """
    out: list[dict] = []
    page_size = 1000
    offset = 0
    while True:
        batch = select(table, columns, filters=filters, limit=page_size, offset=offset, order=order)
        if not batch:
            break
        out.extend(batch)
...
# Mirrors the per-family note-merge helpers in scripts/pdf/lib_supabase_rows.py.
# The primary comment path goes through that module (via _pdf_public_actor);
# these mirrors keep the raw-notes FALLBACK path from re-introducing several
# quotes for one family when the PDF helper has no data for the actor.
...
def _actor_name_key(name: Any) -> str:
    ...
    return " ".join(
        # Only collapse 3+ identical repeats so real doubles (Patterson, Griffith, etc.) survive.
        re.sub(r"([a-z])\1{2,}", r"\1", token) if len(token) >= 5 else token
        for token in tokens
    )
...
def _load_row_review_map() -> dict[str, str]:
    try:
        rows = select_all("court_actor_row_review", ["row_id", "decision"], order="row_id.asc")
    except SupabaseError as e:
...
def _load_hidden_submission_ids() -> set[str]:
    try:
        rows = select_all(
            "admin_review_decisions",
            ["source_id"],
            filters=["source_table=eq.survey_submissions", "decision=in.(superseded,delete)"],
        )
    except SupabaseError as e:
        if e.status in (404, 406, 42703) or "admin_review_decisions" in e.body or "42P01" in e.body:
            return set()
        raise
    return {str(row.get("source_id")) for row in rows if row.get("source_id")}


def _load_submission_visibility_map(submission_ids: list[str], config: dict) -> dict[str, dict]:
    block = config["survey_submissions"]
    ids = list(dict.fromkeys(str(sid) for sid in submission_ids if sid))
    if not ids:
        return {}
    out: dict[str, dict] = {}
    for i in range(0, len(ids), 100):
        ...
        try:
            rows = select(
                ...
                [
                    block["id_column"],
                    block["approved_column"],
                    block["permission_column"],
                ],
                filters=[f"{block['id_column']}=in.({id_csv})"],
                limit=len(chunk),
            )
        except SupabaseError as e:
            if DEBUG:
                print(f"[supabase] submission visibility query failed: {e.status}", file=sys.stderr)
            continue
        for row in rows:
            sid = row.get(block["id_column"])
            if sid:
                out[str(sid)] = {
                    "approved": row.get(block["approved_column"]),
                    "permission_to_share": row.get(block["permission_column"]),
                }
    return out
...
def clean_public_text(text: str, max_words: int, *, preserve_full_text: bool = False) -> str:
    """Conservative sanitizer for draft share graphics.

    Anything with obvious PII, case identifiers, or addresses is dropped so the
    renderer uses a placeholder instead of publishing it. Do not drop an entire
    family-reported actor comment just because it includes violence-related
    words; those comments may already be public in the state PDF, and the share
    renderer selects a short excerpt for the slide.
    """
    if not text:
        return ""
    s = " ".join(str(text).split())
    # Internal extraction-provenance markers ("[extracted_ai] ...",
    # "[extracted_regex] ...") mean the text was auto-extracted, not written by
    # the family. Those rows are admin-only until promoted, so never publish.
    if re.search(r"\[extracted[\w-]*\]", s, flags=re.IGNORECASE):
        return ""
    risky_patterns = [
        r"\b[\w.+-]+@[\w.-]+\.\w+\b",
...
def _load_alias_clusters() -> dict[str, set[str]]:
    """Load admin-confirmed same_actor alias decisions and return a lookup
    of '<name_key>|<location_key>' -> set(all variant name_keys in cluster).

    Mirrors lib_supabase_rows._load_court_actor_alias_resolver and the TS
    AliasResolver so the share page merges the exact same spelling variants
    the public API and PDFs do (e.g. 'Berman' + 'Burman'). Surname ILIKE
    alone only catches one spelling. Cached per-process. Returns {} when the
    table is missing so pre-migration builds still run.
    """
    global _ALIAS_CLUSTERS_CACHE
    if _ALIAS_CLUSTERS_CACHE is not None:
        return _ALIAS_CLUSTERS_CACHE
    out: dict[str, set[str]] = {}
    try:
        decisions = select_all(
            "court_actor_alias_decisions",
            ["location_key", "decision", "name_keys"],
            filters=["decision=eq.same_actor"],
        )
    except SupabaseError as e:
        if DEBUG:
            print(f"[supabase] alias decisions query failed: {e.status}", file=sys.stderr)
        _ALIAS_CLUSTERS_CACHE = {}
        return _ALIAS_CLUSTERS_CACHE
    for d in decisions:
        location_key = d.get("location_key") or ""
        name_keys = [nk for nk in (d.get("name_keys") or []) if nk]
        if len(name_keys) < 2:
            continue
        keyset = set(name_keys)
        for nk in name_keys:
            out[f"{nk}|{location_key}"] = keyset
    _ALIAS_CLUSTERS_CACHE = out
    return out


def _alias_variant_name_keys(
    canonical_for_match: Any, rows: list[dict], state_abbr: str, block: dict
) -> set[str]:
    """All confirmed-variant name_keys for the actor being resolved, looked up
    from the canonical name and every already-matched row. Empty when this
    actor is not part of any same_actor cluster."""
    clusters = _load_alias_clusters()
    if not clusters:
        return set()
    location = (state_abbr or "").upper()
    candidate_keys: set[str] = set()
    canonical_key = _actor_name_key(canonical_for_match)
    if canonical_key:
        candidate_keys.add(canonical_key)
    for r in rows:
        rk = _actor_name_key(r.get(block["name_column"]) or "")
        if rk:
            candidate_keys.add(rk)
    variant_keys: set[str] = set()
    for ck in candidate_keys:
        cluster = clusters.get(f"{ck}|{location}")
        if cluster:
            variant_keys |= cluster
    return variant_keys
...
def resolve_actor(
    ...
    # Admin alias expansion: court_actor_alias_decisions records spelling
    # variants an admin has confirmed are the same person (e.g. Berman/Burman).
    # The public API and PDFs merge these; the surname ILIKE above only catches
    # one spelling, so pull every confirmed variant in this location explicitly.
    # Gated entirely on the admin decision — unrelated same-surname actors are
    # never swept in.
    alias_keys = _alias_variant_name_keys(canonical_for_match, rows, state_abbr, block)
    if alias_keys:
        seen_ids = {r.get(block["id_column"]) for r in rows}
        have_keys = {
            _actor_name_key(r.get(block["name_column"]) or "") for r in rows
        }
        missing_surnames = {
            key.split()[-1]
            for key in alias_keys - have_keys
            if key.split()
        }
        for surname in missing_surnames:
            variant_filters = [f"{block['name_column']}=ilike.*{surname}*"]
            if state_abbr:
                variant_filters.append(f"{block['state_code_column']}=eq.{state_abbr}")
            try:
                variant_rows = select(table, ["*"], filters=variant_filters, order=f"{block['created_column']}.desc")
            except SupabaseError as e:
                variant_rows = []
                if DEBUG:
                    print(f"[supabase] alias variant expansion failed: {e.status}", file=sys.stderr)
            for r in variant_rows:
                rid = r.get(block["id_column"])
                if rid in seen_ids:
                    continue
                if _actor_name_key(r.get(block["name_column"]) or "") in alias_keys:
                    rows.append(r)
                    seen_ids.add(rid)
    ...
    public_comment_row_ids = [r[block["id_column"]] for r in public_comment_rows if r.get(block["id_column"])]
    raw_notes = [r[block["notes_column"]] for r in public_rows if r.get(block["notes_column"])]
    raw_public_notes = [r[block["notes_column"]] for r in public_comment_rows if r.get(block["notes_column"])]
    raw_public_note_items = [
        {
            "note": r[block["notes_column"]],
            "submission_id": str(r.get(block["submission_link_column"]) or ""),
            "created_at": str(r.get(block["created_column"]) or ""),
        }
        for r in public_comment_rows
        if r.get(block["notes_column"])
    ]
    ...
    return {
        ...
        "_public_comment_row_ids": public_comment_row_ids,
        "_submission_ids": submission_ids,
        "_raw_notes": raw_notes,
        "_raw_public_notes": raw_public_notes,
        "_raw_public_note_items": raw_public_note_items,
        "_mention_count": len(public_rows),
    }, None
...
def resolve_public_actor_family_count(
    ...
    try:
        # Paged fetch: CA/FL/TX already have thousands of form_direct rows and
        # a hard limit here silently understated family counts once exceeded.
        # id tiebreak keeps page boundaries stable when created_at collides.
        rows = select_all(
            block["table"],
            [
                "*",
            ...
            order=f"{block['created_column']}.asc,id.asc",
        )
    except SupabaseError as e:
    ...
    for row in rows:
        if not row.get("role") or not row.get("name"):
            continue
        if str(row.get("submission_id") or "") in hidden_submission_ids:
            continue
        submission = _joined_submission(row)
...
# standwithmeg-registry-clean/scripts/pdf/generate_state_pdf.py
...
def resolve_clean_state_medians(state: str, state_abbr: str, config: dict) -> tuple[dict, list[str]]:
    ...
    try:
        rows = select_all(
            ...
            order=f"{block['created_column']}.desc,id.asc",
        )
    except SupabaseError as e:
...
PDF_STATS_HELPER = Path(
    os.environ.get(
        ...
        "/Users/meghannmiller/Code/standwithmeg-registry-clean/scripts/pdf/state_stats_for_share.py",
    )
)
    ...
    if PUBLIC_ACTORS_CACHE_FILE is not None and PUBLIC_ACTORS_CACHE_FILE.exists():
        ...
        if target_tokens:
            ...
            candidates = [
                ...
                if surname in set(_actor_loose_name_key(actor.get("name") or "").split())
            ]
            if len(candidates) == 1:
                return candidates[0], None
...
def resolve_family_reports(actor: dict, config: dict) -> tuple[list[dict], int | None, str | None]:
    ...
    hidden_submission_ids = _load_hidden_submission_ids()
    submission_ids = [
        sid for sid in (actor.get("_submission_ids") or [])
        if str(sid) not in hidden_submission_ids
    ]
    ...
    out: list[dict] = []
    for r in rows:
        quote = clean_public_text(
            r.get(block["quote_column"]) or "",
            max_words,
...
def resolve_public_comments(actor: dict, config: dict) -> tuple[list[dict], str | None]:
    ...
    actor_row_ids = actor.get("_public_comment_row_ids")
    if actor_row_ids is None:
    ...
    # Skip entirely when the admin has curated every public row via
    # comment_merges. comments_in_set_d (pick) is larger than the family
    # count for most actors, so without this guard the un-curated originals
    # get padded in right alongside the curated quotes — resurfacing exactly
    # the fragments/name-calling the curation was meant to remove.
    public_row_count = len(actor.get("_public_comment_row_ids") or [])
    fully_curated = bool(merged) and public_row_count > 0 and len(merged) >= public_row_count
    if len(out) < pick and not fully_curated:
        # One quote per submission: a family that filed several rows (or
        # resubmitted with reworded notes) gets ONE merged note here, never
        # several near-identical quotes side by side on the slide.
        note_items = actor.get("_raw_public_note_items")
        if note_items:
            merged_notes = _merge_note_items_per_submission(note_items)
        else:
...
def resolve_movement_total(config: dict) -> tuple[int | None, str | None]:
    ...
    in standwithmeg-registry-clean does exactly this:
...
def write_spec(base: Path, resolved: Resolved, args: argparse.Namespace) -> Path:
    ...
    if args.display_name:
        ...
        if display_first and display_last == last_name:
            display_first_key = _actor_name_key(display_first)
            first_name_key = _actor_name_key(first_name)
            display_has_more_given_names = (
                first_name_key
    ...
    # Load per-actor display overrides. Looks in two places, in order:
    #   1. PROJECT_ROOT/_scripts/actor_overrides.json — court-actor-posts working
    #      copy (where local manual runs read from).
    #   2. SCRIPT_DIR/actor_overrides.json — committed in this repo next to the
    #      script, so the CI bot also applies the same corrections.
    # Used for spelling fixes, role corrections, etc. that should survive future
    # Supabase rebuilds without modifying the underlying database.
    overrides_candidates = [
        PROJECT_ROOT / "_scripts" / "actor_overrides.json",
        SCRIPT_DIR / "actor_overrides.json",
    ]
    overrides_path = next((p for p in overrides_candidates if p.exists()), overrides_candidates[0])
    ...
    # Extract non-actor-dict directives BEFORE the dict update so they don't
    # leak into actor_dict. Currently supported:
    #   exclude_quote_substrings: [str] — drop any resolved public_comment
    #     whose text contains one of these substrings (case-insensitive).
    #     Used to strip name-calling / off-topic submissions without round-
    #     tripping a Supabase edit. Survives every regen.
    quote_excludes: list[str] = [
        s for s in (actor_overrides.pop("exclude_quote_substrings", None) or [])
        if isinstance(s, str) and s.strip()
    ]
    if quote_excludes:
        before = len(resolved.public_comments)
        resolved.public_comments = [
            c for c in resolved.public_comments
            if not any(s.lower() in (c.get("comment_text") or "").lower() for s in quote_excludes)
        ]
        dropped = before - len(resolved.public_comments)
        if dropped:
            print(f"  dropped {dropped} public_comment(s) for {args.actor} via exclude_quote_substrings", file=sys.stderr)

    # comment_rewrites: [{match, replace}] — replace the FULL text of any
    # resolved public_comment that contains `match` (case-insensitive) with
    # `replace`. Used to de-identify a family comment for safety (retaliation
    # risk) while keeping a court-backed point, without a Supabase round-trip:
    # the public spec.json, the slide, and the PDF then carry only the softened
    # text. Survives every regen. First matching rule wins per comment.
    comment_rewrites = [
        r for r in (actor_overrides.pop("comment_rewrites", None) or [])
    ...
    if comment_rewrites:
        rewritten = 0
        new_comments = []
        for c in resolved.public_comments:
            text = c.get("comment_text") or ""
            for r in comment_rewrites:
                if r["match"].lower() in text.lower():
                    c = {**c, "comment_text": r["replace"]}
                    rewritten += 1
                    break
            new_comments.append(c)
        resolved.public_comments = new_comments
        if rewritten:
            print(f"  rewrote {rewritten} public_comment(s) for {args.actor} via comment_rewrites", file=sys.stderr)

    # replace_public_comments: [str] — replace the resolved public quote rows
    # with a curated, publishable set for one actor. This is intentionally
    # per-actor and opt-in: it is for cases where one merged family note contains
    # multiple concrete clauses but also unsafe/noisy language that should not
    # appear on a public slide. The replacement flows into spec.json, share
    # slides, and state PDFs on future regens.
    replacement_comments: list[str] = [
        s.strip()
        for s in (actor_overrides.pop("replace_public_comments", None) or [])
        if isinstance(s, str) and s.strip()
    ]
        ...
        resolved.public_comments = [
            {
                "comment_text": text,
                "author": "Anonymous parent",
                "source": "actor_overrides",
                "court_or_county": resolved.actor.get("court_or_county") or args.county,
            }
            for text in replacement_comments
        ]
        resolved.unresolved = [
            item for item in resolved.unresolved
            if not str(item).startswith("public_comments:")
        ]
        print(f"  replaced public_comments for {args.actor} via replace_public_comments", file=sys.stderr)

    # quote_char_budget: int — widen the per-quote character budget on the
    # multi-quote family-quotes frame so quotes render fuller instead of
    # truncating to the default count-based budget. Opt-in per actor; written
    # into spec.render so render_spotlight picks it up on every regen.
    quote_char_budget = actor_overrides.pop("quote_char_budget", None)
    if not (isinstance(quote_char_budget, int) and quote_char_budget > 0):
        quote_char_budget = None

    # prefer_quote_substrings: [str] — when multiple safe quote clauses exist,
    # prefer the one containing one of these phrases. Used for cases where the
    # most important family-reported clause is the second sentence, not the
    # first clean sentence the deterministic selector would otherwise choose.
    prefer_quote_substrings: list[str] = [
        s for s in (actor_overrides.pop("prefer_quote_substrings", None) or [])
        if isinstance(s, str) and s.strip()
    ]
    ...
    # above. Those already prefer the LIVE court_actors name (so an admin
    # spelling fix in Supabase reaches the slide on the next regen, matching the
    # PDF and registry) while preserving a more-complete manifest name when it
    # adds given names (middle initial, nickname). Falls back to the raw resolved
    # or manifest name if the split produced nothing. actor_overrides.display_name
    # still wins via the actor_dict.update(actor_overrides) below.
    computed_display_name = " ".join(
        part for part in [title, first_name, last_name] if part and part.strip()
    ).strip()
    actor_dict = {
        "slug": args.actor,
        "display_name": computed_display_name or resolved.actor.get("name") or args.display_name,
        "title": title,
    ...
    spec = {
        ...
        "guardrails": {
            ...
            "footer": "Family-reported submissions.",
            "strike_through_reserved_for": ["isolated", "dismissed", "ignored"],
            "no_minors": True,
            "no_case_numbers": True,
        },
        "public_share": {
            "recommended_route": f"/court-actors/{(args.state_abbr or '').lower()}/{args.actor}/share.html",
            "draft_only": True,
        },
        "render": {
            **({"quote_char_budget": quote_char_budget} if quote_char_budget else {}),
            **({"prefer_quote_substrings": prefer_quote_substrings} if prefer_quote_substrings else {}),
        },
