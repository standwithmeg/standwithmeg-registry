...
function getApiKey(): string {
  // Read directly from .env.local as fallback if Next.js env loading fails
...
export async function POST(request: Request) {
  try {
    ...
    const { documentText, question, state, caseType, conversationHistory, caseContext } = body;
    ...
    const vaultDocs = Array.isArray(documentText)
      ? documentText.map((d: { name: string; text: string }) => d.text)
    ...
    if (documentText) {
      ...
      if (Array.isArray(documentText)) {
        for (let i = 0; i < documentText.length; i++) {
          ...
          const text = (doc.text || "").slice(0, 12000);
    ...
    // Include conversation history if provided (for follow-ups)
