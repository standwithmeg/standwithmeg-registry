...
function redirectWithClearedCookie(url: string): Response {
...
export async function GET(request: Request) {
  ...
  if (errorParam || !code || !state) {
    ...
    return redirectWithClearedCookie(
      `${appUrl}/vaults?google_error=${encodeURIComponent(reason)}`
    );
  }

  // Validate that the returned state matches the random nonce we stored
  // in the HTTP-only cookie, and recover the user_id bound to this flow.
  const expected = validateOAuthState(request);
  if (!expected || expected.state !== state) {
    return redirectWithClearedCookie(
      `${appUrl}/vaults?google_error=invalid_state`
    );
  }

  const userId = expected.userId;

  // Defense-in-depth: verify the user actually has an active Supabase session.
  // The state cookie binds the nonce to a userId, but that cookie could be
  // stolen or replayed. Re-checking auth.getUser() confirms the session is
  // still alive at callback time and matches the userId bound into the state.
  const authClient = await createServerSupabaseClient();
  const { data: { user }, error: sessionError } = await authClient.auth.getUser();
  if (sessionError || !user || user.id !== userId) {
    return redirectWithClearedCookie(
      `${appUrl}/vaults?google_error=invalid_session`
    );
  }
  ...
  try {
    ...
    if (!tokenRes.ok) {
      ...
      return redirectWithClearedCookie(
        `${appUrl}/vaults?google_error=token_exchange_failed`
      );
    }
  ...
  if (!tokenData.refresh_token) {
    ...
    return redirectWithClearedCookie(
      `${appUrl}/vaults?google_error=no_refresh_token`
    );
  }
  ...
  if (upsertError) {
    ...
    return redirectWithClearedCookie(`${appUrl}/vaults?google_error=db_error`);
  }

  return redirectWithClearedCookie(`${appUrl}/vaults?google_connected=1`);
}
