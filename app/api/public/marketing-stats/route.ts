...
interface StaticMarketingFallback {
  total: number;
...
async function loadFinancialSnapshot(): Promise<{
  total_financial_loss?: number;
  by_state?: Record<string, number>;
}> {
  try {
    const filePath = path.join(process.cwd(), "public", "state-reports", "financials.json");
...
async function loadStaticMarketingFallback(): Promise<StaticMarketingFallback> {
  const [stateReports, financials] = await Promise.all([
    loadStateReports(),
    loadFinancialSnapshot(),
  ]);
  const lossByState = financials.by_state ?? {};
  const byState = stateReports.map(report => ({
    state: report.state,
    is_us: Object.prototype.hasOwnProperty.call(US_STATE_NAMES, report.state),
    total_submissions: Number(report.submissions) || 0,
    total_financial_loss: typeof lossByState[report.state] === "number" ? lossByState[report.state] : null,
    avg_months_lost: null,
    pro_se_count: 0,
  }));
  const total = byState.reduce((sum, row) => sum + row.total_submissions, 0);
  const snapshotTotalLoss = typeof financials.total_financial_loss === "number"
    ? financials.total_financial_loss
    : byState.reduce((sum, row) => sum + (Number(row.total_financial_loss) || 0), 0);
  return {
    total,
    snapshot_total_financial_loss: snapshotTotalLoss,
    by_state: byState,
  ...
  for (const table of tables) {
    ...
    while (true) {
      const { data, error } = await supabase
        ...
        // Stable unique sort key — required for correct range pagination.
...
export async function GET() {
  try {
    ...
    const [byStateResult, agg, stateReports, topCounties] = await withSupabaseReadTimeout(
      Promise.all([
        supabase
    ...
    const fallback = await loadStaticMarketingFallback();
    const rows = fallback.by_state;
    const stateReports = fallback.state_reports;
    const usStates = new Set(rows.filter(row => row.is_us).map(row => row.state));
    const globalCountries = rows
      .filter(row => !row.is_us)
      .map(row => ({
        slug: slugify(row.state),
        country: row.state,
        region: COUNTRY_REGION[slugify(row.state)] ?? "Global",
      }));
    const parentingMonths = rows.reduce(
      (sum, row) => sum + (Number(row.avg_months_lost) || 0) * (Number(row.total_submissions) || 0),
      0,
    );
    ...
    return Response.json(
      {
        generated_at: new Date().toISOString(),
        total_submissions: fallback.total,
        us_states_count: usStates.size,
        countries_count: (usStates.size > 0 ? 1 : 0) + globalCountries.length,
        total_financial_loss: fallback.snapshot_total_financial_loss ?? 0,
        parenting_time_lost_years: Math.round(parentingMonths / 12),
        pro_se_count: rows.reduce((sum, row) => sum + (Number(row.pro_se_count) || 0), 0),
        report_ready_states: stateReports.length,
        state_reports: stateReports,
        global_countries: globalCountries,
        top_counties: [],
      ...
      {
        // Short-lived: this is the static snapshot fallback. Caching it for the
        // success-path window (s-maxage=900 + SWR=3600) would pin degraded data
        // at the edge for over an hour after Supabase recovers.
        headers: {
