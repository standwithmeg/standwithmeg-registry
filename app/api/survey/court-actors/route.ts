...
import { publicManifestAssetExists } from "../../../../lib/court-actor-deploy";
...
async function manifestEntryWithLiveAssets(entry: ManifestEntry): Promise<ManifestEntry> {
  const [photoExists, shareExists] = await Promise.all([
    publicManifestAssetExists(entry.photo_url),
    publicManifestAssetExists(entry.share_url),
  ]);
  return {
    ...entry,
    photo_url: photoExists ? entry.photo_url : null,
    share_url: shareExists ? entry.share_url : null,
  };
}
...
async function loadSpotlightManifest(): Promise<ManifestLookups> {
  try {
    ...
    for (const rawEntry of data.actors || []) {
      const entry = await manifestEntryWithLiveAssets(rawEntry);
...
/**
 * Loads admin same_actor decisions so the public page picks up canonical
 * names. Without this, the public site shows whatever spelling the
 * majority of families typed — even when the admin set a different
 * canonical name in Possible Matches.
 */
...
export async function GET(request: Request) {
  try {
    ...
    for (const a of all) {
      ...
      // Apply admin canonical-name decisions from Possible Matches so the
      // public site shows the spelling the admin chose, not whichever
      // spelling more families happened to type.
      ...
      // Use the canonical name (when an alias hit) so all spelling variants
      // collapse to the admin-chosen casing in the casingCounts ranking.
    ...
    // Load the spotlight manifest so we can hand the frontend a photo_url
    // and share_url for any actor whose page has been deployed.
