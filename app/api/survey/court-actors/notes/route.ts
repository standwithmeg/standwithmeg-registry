  ...
  try {
    ...
    while (true) {
      const { data, error } = await sb
        ...
        // Stable sort so .range() pages never skip or duplicate rows at page
        // boundaries — same fix as /api/survey/court-actors.
    ...
    // the merged rows' original notes are hidden. The families still count
    // toward the public threshold.
    ...
    for (const row of matchingRows) {
      ...
      if (rowReviewMap.get(row.id) === "merge_comments" && !mergeEntry) {
        continue;
      }
