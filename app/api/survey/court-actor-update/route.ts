...
export async function POST(request: Request) {
  try {
    ...
    const actorThresholdBefore = rowLocationKey
      ? await loadCourtActorThresholdSnapshot(adminSupabase, rowLocationKey)
      : null;
    ...
    const crossedPublicActorThreshold = await queueStateRegenerationIfCourtActorThresholdCrossed({
      ...
      reason: "court actor update crossed public threshold",
    });
    if (crossedPublicActorThreshold) {
      after(() => refreshPublicActorCache(adminSupabase).catch(err => {
        console.error("public actor cache refresh failed after court actor update threshold crossing:", err);
      }));
    }
