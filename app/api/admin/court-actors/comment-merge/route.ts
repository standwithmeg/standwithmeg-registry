 ...
 *             'merge_comments' for each merged_row_id (so resolveFamilyKey
 *             excludes those rows from family counts and the read paths
 *             skip their original notes).
 ...
 *      → C re-enters family counts; if the family had only 1 family-key
 *        from this email it's a wash, but if C had a different email the
 *        public count goes back up.
  ...
  try {
    ...
    if (reviewError) {
      ...
      return Response.json({
        merge: mergeData,
        warning: `Saved the merge but failed to mark merged rows as merge_comments in row_review: ${reviewError.message}. Family counts may not yet exclude the merged rows.`,
      });
    }
    ...
    if (removedRowIds.length > 0) {
      ...
      if (clearError) {
        ...
        return Response.json({
          merge: mergeData,
          warning: `Saved the merge but failed to clear ${removedRowIds.length} stale merge_comments row_review entr${removedRowIds.length === 1 ? "y" : "ies"} for the unchecked rows: ${clearError.message}. Those rows may still show as not-counting until cleared manually.`,
        });
      }
