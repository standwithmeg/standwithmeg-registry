...
import { DONATION_URL, HOW_TO_USE_ACTORS_VIDEO_EMBED_URL } from "../../../../lib/site-links";
...
function downloadActorsCSV(
  ...
  const headers = ["Name", "Role", "State / Country", "Families", "Status", "County / Court", "Profile Link"];
  const rows = actors.map(a => [
    a.name,
    a.role,
    locationLabel(a),
    a.family_count,
    a.at_threshold ? `Public profile (${a.family_count} families)` : `Needs +${a.needs_more} more`,
    a.county_breakdown,
    a.at_threshold ? `${window.location.origin}${courtActorComplaintPacketUrl(a)}` : "",
  ]);
  ...
  const parts = ["swm-actors"];
  if (filters.stateFilter) parts.push(filters.stateFilter.replace(/[^a-z0-9]+/gi, "-"));
  if (filters.roleFilter) parts.push(filters.roleFilter.replace(/[^a-z0-9]+/gi, "-"));
  if (filters.search) parts.push("search-" + filters.search.replace(/[^a-z0-9]+/gi, "-").slice(0, 30));
  if (filters.thresholdOnly) parts.push("public-only");
...
export function ActorsBrowser({ visitorEmail, visitorSubmissionId, visitorFirstName, onSignOut }: Props) {
  ...
  const [stats, setStats] = useState<{
    total_actors: number;
    total_actor_mentions: number;
    total_families_documented: number | null;
    at_threshold: number;
    states_count: number;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [actorsRes, surveyRes] = await Promise.all([
        ...
        setStats({
          ...
          total_actor_mentions: data.total_actor_mentions ?? data.total_reports,
          total_families_documented: typeof surveyData?.total === "number" ? surveyData.total : null,
    ...
    >
      <div className="max-w-6xl mx-auto px-4 py-8 md:py-12">
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-2 mb-5">
          <div className="flex items-center gap-2.5 self-start">
            ...
            <SponsorCtaButton
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold transition-colors hover:opacity-90"
            />
          </div>
        ...
        >
          ...
          {/* Quick walkthrough video — small click-to-play short for first-time visitors */}
          <div className="mb-6 flex flex-col sm:flex-row items-center gap-4 sm:gap-5">
            <div
              className="w-40 sm:w-44 flex-shrink-0 aspect-[9/16] rounded-xl overflow-hidden"
              style={{ border: "1px solid rgba(201,162,39,0.4)" }}
            >
              <iframe
                className="w-full h-full"
                src={HOW_TO_USE_ACTORS_VIDEO_EMBED_URL}
                title="How to use the Stand With Meg court actor registry"
                loading="lazy"
            ...
            <div className="text-center sm:text-left">
              ...
              <p className="text-white/85 text-sm md:text-base leading-relaxed">
                Under a minute &mdash; see how to find a name and add yourself to their report.
              </p>
            </div>
          </div>
        ...
        >
          ...
          <a
            ...
            className="shrink-0 inline-flex items-center justify-center px-6 py-3 rounded-lg font-black text-sm tracking-wide hover:opacity-90 transition-opacity"
          ...
          <div className="mb-6">
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
              <StatCard
                ...
                value={stats.total_families_documented === null ? "—" : stats.total_families_documented.toLocaleString()}
                help="Total family submissions in the movement dashboard."
              />
              <StatCard
                label="Actor Mentions"
                value={stats.total_actor_mentions.toLocaleString()}
                help="Every person named across those family reports."
              />
              <StatCard
                label="Name Groups"
                value={stats.total_actors.toLocaleString()}
                help="Same name, role, and location grouped together."
              />
              <StatCard
                label="Public Profiles"
                value={stats.at_threshold.toString()}
                help={`${threshold}+ different families named the same pattern.`}
                accent
              />
              <StatCard
                label="Locations"
                value={(stats.locations_count ?? stats.states_count).toString()}
                help="States and countries represented in the registry."
              />
            </div>
            <div
              className="mt-3 rounded-xl px-4 py-3 text-sm leading-relaxed text-white/75"
              style={{ backgroundColor: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.10)" }}
            >
              <span className="font-bold text-white">Why the numbers are different:</span>{" "}
              the real report count is the families documented number. One family
              can name more than one court actor, so actor mentions are higher.
              Those mentions are grouped by name, role, and location; only groups
              named by {threshold}+ different families
              become public profiles.
            </div>
          </div>
        )}

        <ConnectionCirclesCta placement="actors" className="mb-6" />
        ...
        >
          ...
          <div className="md:col-span-2 flex items-end">
            <label className="flex items-center gap-2 text-sm text-white/80 cursor-pointer pb-2">
              ...
              />
              Show only public profiles ({threshold}+ families)
            </label>
          </div>
        </div>
          ...
          <>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
              <div className="text-white/60 text-xs">
                Showing <span className="text-white">{filtered.length.toLocaleString()}</span> of <span className="text-white">{actors!.length.toLocaleString()}</span> name groups
              </div>
              <button
                type="button"
                onClick={() => downloadActorsCSV(filtered, { stateFilter, roleFilter, search, thresholdOnly })}
                className="inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2 text-xs font-black transition-opacity hover:opacity-90"
                style={{ backgroundColor: GOLD, color: NAVY_DEEP }}
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row flex-wrap gap-3">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="px-6 py-3 rounded-lg font-black text-sm tracking-wide text-center hover:opacity-90 transition-opacity"
              style={{ backgroundColor: "#B91C1C", color: "white" }}
            >
              Donate →
            </a>
        ...
        <div className="mt-6 text-white/50 text-xs leading-relaxed">
          ...
          <p>
            <span className="text-white/70 font-semibold">Names go fully public</span> on the <Link href="/report" className="underline" style={{ color: GOLD }}>main dashboard</Link> once {threshold} different families have independently named the same person in the same place.
          </p>
        </div>
      </div>
...
function StatCard({ label, value, help, accent }: { label: string; value: string; help: string; accent?: boolean }) {
  return (
    <div
      className="rounded-xl px-4 py-3 text-center flex flex-col justify-center min-h-[112px]"
      style={{
    ...
    >
      ...
      <div className="text-[11px] leading-snug text-white/55 mt-2 normal-case tracking-normal">{help}</div>
    </div>
  );
}
...
function ClaimModal({
  ...
  const actorLocation = actor.state_code || actor.location_key || "";
  const updateUrl = `/court-actor-update?submission=${encodeURIComponent(visitorSubmissionId)}&actor_name=${encodeURIComponent(actor.name)}&actor_role=${encodeURIComponent(primaryRole)}&actor_county=${encodeURIComponent(actor.county_breakdown.split(" (")[0] || "")}&actor_state=${encodeURIComponent(actorLocation)}`;
  return (
    <div
    ...
    >
      ...
      >
        ...
        <p className="text-white/70 text-sm mb-5">{actor.role} · {locationLabel(actor)}</p>
