...
import { fetchCourtActorsForState, type PublicActor } from "./CourtActorPanel";
...
export function StateQuoteModal({
  state,
  isUs,
  totalSubmissions,
  resource,
  actorThreshold = COURT_ACTOR_PUBLIC_THRESHOLD,
  onClose,
}: Props) {
        ...
        <div className="overflow-y-auto flex-1 px-6 py-4 space-y-4">
          ...
          {!loading && actors.length > 0 && (
            <div className="mt-6 rounded-xl overflow-hidden" style={{ border: "1px solid rgba(201,162,39,0.2)" }}>
              ...
              <div>
                {actors.map((a, i) => (
                  <div key={`${a.state_code ?? "NA"}-${a.location_key ?? "NA"}-${a.name}-${a.court_or_county ?? ""}`} className="px-4 py-3 flex items-center justify-between gap-3"
                    ...
                    <div className="text-xs font-bold whitespace-nowrap px-2.5 py-1 rounded-md"
                      ...
                      {a.count} {a.count === 1 ? "family" : "families"}
                    </div>
                  </div>
                ))}
              </div>
