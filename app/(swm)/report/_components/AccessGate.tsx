  ...
  return (
    <div className="min-h-screen flex flex-col relative" style={{ backgroundColor: "#0F1E30" }}>
      ...
      <div className="relative z-10 flex-1 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-3xl">
          ...
          <div className="text-center mb-8">
            ...
            <p className="author text-xs font-semibold" style={{ color: "rgba(245,245,245,0.72)" }}>
              By <a href="/about" target="_blank" rel="author noopener noreferrer" className="underline underline-offset-2">Stand With Meg</a>
            </p>
            <p className="mt-3 text-sm max-w-md mx-auto" style={{ color: "rgba(245,245,245,0.72)" }}>
          ...
          <ConnectionCirclesCta placement="report" className="mb-6" />
          ...
          <form onSubmit={handleSubmit} className="rounded-2xl p-8 space-y-5"
            ...
            <p className="text-xs text-center" style={{ color: "rgba(245,245,245,0.72)" }}>
            ...
            <div className="rounded-xl p-4"
              ...
              <h2 className="text-sm font-black text-white">What this report shows</h2>
              <p className="mt-2 text-xs leading-relaxed" style={{ color: "rgba(245,245,245,0.76)" }}>
                The dashboard summarizes family-submitted registry data by state, including reported system type,
                financial loss, time away from children, pro se status, public quotes, and court-actor patterns.
                It is built for families, advocates, journalists, and lawmakers who need aggregate evidence without
                exposing private submitter identities.
              </p>
              <p className="mt-2 text-xs leading-relaxed" style={{ color: "rgba(245,245,245,0.76)" }}>
                Submissions are lived experiences and allegations unless a public source supports stronger wording.
                Public naming follows threshold and review rules so one family&apos;s story does not become a public
                actor page by itself.
              </p>
              <p className="mt-2 text-xs leading-relaxed" style={{ color: "rgba(245,245,245,0.76)" }}>
                Access requests are logged to reduce scraping and misuse. The goal is to keep the numbers available
                for serious reform work while protecting the people who supplied the records behind those numbers.
              </p>
              <p className="mt-2 text-xs leading-relaxed" style={{ color: "rgba(245,245,245,0.76)" }}>
                The public view is intentionally aggregate. It helps visitors understand where reports are coming from,
                which states have enough submissions for a downloadable report, and how repeated court-actor patterns
                are reviewed before they become visible outside the admin workflow.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-[11px] uppercase tracking-widest font-bold" style={{ color: "rgba(245,245,245,0.68)" }}>
                or continue to data
              </span>
              <div className="h-px flex-1" style={{ backgroundColor: "rgba(255,255,255,0.1)" }} />
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                <label htmlFor="dashboard-first-name" className="block text-sm font-semibold mb-1.5" style={{ color: "rgba(245,245,245,0.8)" }}>
                  First Name <span className="text-red-400">*</span>
                </label>
                <input id="dashboard-first-name" type="text" value={firstName} onChange={e => setFirstName(e.target.value)} required
                  placeholder="First name"
                  className="w-full rounded-lg px-4 py-3 text-white text-sm focus:outline-none"
                  style={inputStyle} />
              </div>
              <div>
                <label htmlFor="dashboard-last-name" className="block text-sm font-semibold mb-1.5" style={{ color: "rgba(245,245,245,0.8)" }}>
                  Last Name <span className="text-red-400">*</span>
                </label>
                <input id="dashboard-last-name" type="text" value={lastName} onChange={e => setLastName(e.target.value)} required
            ...
            <div>
              <label htmlFor="dashboard-organization" className="block text-sm font-semibold mb-1.5" style={{ color: "rgba(245,245,245,0.8)" }}>
                Organization / Outlet <span className="text-xs font-normal" style={{ color: "rgba(245,245,245,0.7)" }}>(optional)</span>
              </label>
              <input id="dashboard-organization" type="text" value={organization} onChange={e => setOrganization(e.target.value)}
            ...
            {/* Email */}
            <div>
              <label htmlFor="dashboard-email" className="block text-sm font-semibold mb-1.5" style={{ color: "rgba(245,245,245,0.8)" }}>
                Email Address <span className="text-red-400">*</span>
              </label>
              <input id="dashboard-email" type="email" value={email} onChange={e => setEmail(e.target.value)} required
            ...
            {/* State */}
            <div>
              <label htmlFor="dashboard-state-interest" className="block text-sm font-semibold mb-1.5" style={{ color: "rgba(245,245,245,0.8)" }}>
                State of Interest
              </label>
              <select id="dashboard-state-interest" value={state} onChange={e => setState(e.target.value)}
            ...
            {/* Role */}
            <div>
              <label htmlFor="dashboard-role" className="block text-sm font-semibold mb-1.5" style={{ color: "rgba(245,245,245,0.8)" }}>
                Your Role <span className="text-red-400">*</span>
              </label>
              <select id="dashboard-role" value={role} onChange={e => setRole(e.target.value)} required
                className="w-full rounded-lg px-4 py-3 text-white text-sm focus:outline-none appearance-none"
                style={inputStyle}>
                {ROLES.map(([val, label]) => (
            ...
            {/* Reason */}
            <div>
              <label htmlFor="dashboard-reason" className="block text-sm font-semibold mb-1.5" style={{ color: "rgba(245,245,245,0.8)" }}>
                Reason for Access <span className="text-xs font-normal" style={{ color: "rgba(245,245,245,0.7)" }}>(optional)</span>
              </label>
              <textarea id="dashboard-reason" value={reason} onChange={e => setReason(e.target.value)} rows={2}
          ...
          {/* Attribution */}
          <p className="text-center text-xs mt-6 px-4 leading-relaxed" style={{ color: "rgba(245,245,245,0.68)" }}>
      ...
      <footer className="relative z-10 text-center px-6 py-5 text-xs border-t"
        style={{ color: "rgba(245,245,245,0.62)", borderColor: "rgba(255,255,255,0.12)" }}>
