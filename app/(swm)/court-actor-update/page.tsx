...
export default function CourtActorUpdatePage() {
  ...
  useEffect(() => {
    ...
    if (prefName || prefRole || prefCounty) {
      ...
      const roleMatch = prefRole ? courtActorRoleOptionOrOther(prefRole) : "";
      setActors([{
        role: roleMatch,
        name: prefName,
        court: prefCounty,
  ...
  return (
    <main className="min-h-screen px-5 py-10" style={{ backgroundColor: "#F7F5F0" }}>
      <section className="mx-auto max-w-3xl rounded-2xl overflow-hidden bg-white shadow-xl"
        ...
        <div className="px-6 py-6 text-white" style={{ backgroundColor: "#0F1E30" }}>
          ...
          <p className="mt-3 text-xs font-semibold text-white/70">
            Published by Stand With Meg for registry submitters correcting or adding factual actor details.
          </p>
        </div>

        <div className="p-6 space-y-5">
          <div className="rounded-xl bg-[#f7f5f0] p-4 text-sm leading-relaxed text-gray-700"
            style={{ border: "1px solid #E5E7EB" }}>
            <h2 className="text-base font-black" style={{ color: NAVY }}>When to use this page</h2>
            <p className="mt-2">
              Use the court actor update form when you already submitted the Family Rights Survey and need to add a judge,
              GAL, attorney, evaluator, caseworker, agency, court, or county that belongs with that same submission.
              This keeps the registry from double-counting one family while still letting the admin review team improve
              actor names, roles, counties, and notes.
            </p>
            <p className="mt-2">
              If you have not submitted a survey yet, start with the <a href="/survey" className="font-bold text-[#8a6d16] hover:underline">Family Rights Survey</a>.
              If you are trying to understand who is already public, use the <a href="/actors" className="font-bold text-[#8a6d16] hover:underline">Court Actor Registry</a>.
            </p>
          </div>

          <div className="rounded-xl bg-white p-4 text-sm leading-relaxed text-gray-700"
            style={{ border: "1px solid rgba(201,162,39,0.35)" }}>
            <h2 className="text-base font-black" style={{ color: NAVY }}>Privacy and review rules</h2>
            <p className="mt-2">
              Stand With Meg uses this update to match your actor details to your existing submission email and private
              submission ID. Your identity, children&apos;s names, address, case number, and private documents should not be
              entered here. Public court-actor pages are pattern summaries, not individual family disclosures.
            </p>
            <p className="mt-2">
              Actor names are not automatically published from this form. Updates go into the review workflow and public
              visibility still depends on trusted family reports meeting the registry threshold.
            </p>
          </div>
          ...
          {sent ? (
            ...
            <>
              <div>
                <label htmlFor="court-actor-update-email" className="block text-xs font-black uppercase tracking-wide mb-1" style={{ color: NAVY }}>
                  Email used on your original survey <span className="text-red-700">*</span>
                </label>
                <input
                  id="court-actor-update-email"
                  type="email"
                  value={email}
              ...
              {actors.map((actor, index) => (
                <div key={index} className="rounded-xl p-4 bg-gray-50"
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    <div>
                      <label htmlFor={`court-actor-update-role-${index}`} className="block text-xs font-bold uppercase tracking-wide mb-1 text-gray-600">
                        Role <span className="text-red-700">*</span>
                      </label>
                      <select
                        id={`court-actor-update-role-${index}`}
                        value={actor.role}
                      ...
                      >
                        ...
                        {COURT_ACTOR_ROLE_OPTIONS.map(role => <option key={role} value={role}>{role}</option>)}
                      </select>
                    </div>
                    <div>
                      <label htmlFor={`court-actor-update-name-${index}`} className="block text-xs font-bold uppercase tracking-wide mb-1 text-gray-600">
                        Name <span className="text-red-700">*</span>
                      </label>
                      <input
                        id={`court-actor-update-name-${index}`}
                        type="text"
                        value={actor.name}
                  ...
                  <div className="mt-3">
                    <label htmlFor={`court-actor-update-court-${index}`} className="block text-xs font-bold uppercase tracking-wide mb-1 text-gray-600">
                      Court or county <span className="font-normal normal-case text-gray-500">(optional)</span>
                    </label>
                    <input
                      id={`court-actor-update-court-${index}`}
                      type="text"
                      value={actor.court}
                  ...
                  <div className="mt-3">
                    <label htmlFor={`court-actor-update-notes-${index}`} className="block text-xs font-bold uppercase tracking-wide mb-1 text-gray-600">
                      What happened with this court actor? <span className="text-red-700">*</span>
                    </label>
                    <textarea
                      id={`court-actor-update-notes-${index}`}
                      rows={4}
                      value={actor.notes}
              ...
              <button type="button" onClick={addActor}
                ...
                style={{ color: NAVY, border: `1.5px dashed rgba(30,58,95,0.42)`, backgroundColor: "#F8FAFC" }}>
      ...
      <nav className="mx-auto mt-6 flex max-w-3xl flex-wrap justify-center gap-x-4 gap-y-2 text-xs font-semibold text-gray-500">
        <a href="/actors" className="hover:text-gray-800">Court Actor Registry</a>
        <a href="/survey" className="hover:text-gray-800">Family Rights Survey</a>
