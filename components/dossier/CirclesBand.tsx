...
const STAT_ICONS = [
  "M16 11a3 3 0 1 0-3-3 3 3 0 0 0 3 3zm-8 0a3 3 0 1 0-3-3 3 3 0 0 0 3 3zm0 2c-2.3 0-7 1.2-7 3.5V19h14v-2.5c0-2.3-4.7-3.5-7-3.5zm8 0c-.3 0-.6 0-1 .1a4.2 4.2 0 0 1 2 3.4V19h6v-2.5c0-2.3-4.7-3.5-7-3.5z",
  "M12 12a4 4 0 1 0-4-4 4 4 0 0 0 4 4zm0 2c-2.7 0-8 1.3-8 4v2h16v-2c0-2.7-5.3-4-8-4z",
  "M20 2H4a2 2 0 0 0-2 2v18l4-4h14a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2z",
];

interface CirclesBandProps {
  /** Real member stats — tiles render only when provided (never fixture numbers on the live site). */
  stats?: { connected: number; handles: number; messagesToday: number };
}
 ...
 */
export function CirclesBand({ stats }: CirclesBandProps = {}) {
  const statRows: Array<[string, string, string]> | null = stats
    ? [
        [String(stats.connected), "Connected", STAT_ICONS[0]],
        [String(stats.handles), "Handles", STAT_ICONS[1]],
        [String(stats.messagesToday), "Messages today", STAT_ICONS[2]],
      ]
    : null;
  return (
    <aside className="panel p-8 md:p-10" aria-label="Connection Circles">
      ...
      <div className="panel p-6 md:p-7" style={{ borderColor: "color-mix(in srgb, var(--action-red) 55%, transparent)" }}>
        <p className="eyebrow mb-5 flex items-center gap-3">
          What it looks like inside <span aria-hidden style={{ width: 34, borderTop: "2px solid var(--action-red)" }} />
        </p>
        {MEMBERS.map((m) => (
        ...
        <p className="disclaimer-strip mt-4">Illustrative example — real rooms and handles stay private.</p>
      </div>
    </aside>
  );
}
